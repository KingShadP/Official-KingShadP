...
try {
  ...
  if (!fs.existsSync(outPath)) {
    console.log('// Note: "out" directory not found. This is a full-stack server-side app, so static export copy is bypassed.');
    console.log('// === BUILD FINALIZATION COMPLETED SUCCESSFULLY FOR FULL-STACK ===');
    process.exit(0);
  }
  ...
  // Generate a robust 404.html by copying index.html for static hosting SPA support
  const indexPath = path.join(outPath, 'index.html');
  const fallbackPath = path.join(outPath, '404.html');
  if (fs.existsSync(indexPath)) {
    console.log('// Generating 404.html fallback for SPA routing...');
    fs.copyFileSync(indexPath, fallbackPath);
  } else {
    console.warn('// Warning: index.html not found, skipping 404.html generation');
  }
